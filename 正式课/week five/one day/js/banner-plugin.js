        ...
        focusEvent: function () {
            this.focusList.forEach((item, index) => {
                item['on' + this.focusEventType] = () => {
                    if (this.curOnclick === index) return;
                    if (index === 0 && this.stepIndex === this.slideList.length - 1) return ;
                    this.stepIndex = index;
                    this.directionMove();
                    this.changeFocus();
                    this.curOnclick = this.stepIndex;
                }
            })
        },
        arrowEvent: function () {
            this.arrowRight.onclick = this.autoMove.bind(this);
            this.arrowLeft.onclick = () => {
                ...
                if(this.direction==='horizontal'){
                    ...
                    if(this.stepIndex<0){
                        this.stepIndex=this.slideList.length-1;
                    }
                    this.slideList.forEach((item,index)=>{
                        utils.css(item,{
                            zIndex:0,
